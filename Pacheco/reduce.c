/* reduce.c -- Parallel Trapezoidal Rule.  Uses 3 calls to MPI_Bcast to distribute input.  Also uses MPI_Reduce to compute final sum.
 *
 * Input: 
 ...
 * Notes:  
 *    1.  f(x) is hardwired.
 *    2.  the number of processes (p) should evenly divide the number of trapezoids (n).
 *
...
// MPI_Bcast() from all processes and must ahve same count and data_type for all calls to eliminate the need of status. It does not involve use of tags. And hence cant be received by MPI_Recv()
	...
	// count - 1 means 1 memory loaction of type MPI_FLOAT ( consecutive 8 bytes )
...
{	int my_rank, p, source, dest = 0, tag = 0, n, local_n;
	...
	// no need to again pass the value of individual processed integral to process 0
