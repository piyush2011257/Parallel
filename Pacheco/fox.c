...
{	int old_rank, dimensions[2], wrap_around[2], coordinates[2], free_coords[2];
...
void Fox( int n/* in  */, GRID_INFO_T* grid/* in  */, LOCAL_MATRIX_T* local_A/* in  */, LOCAL_MATRIX_T* local_B/* in  */, LOCAL_MATRIX_T*  local_C/* out */)
{	LOCAL_MATRIX_T*  temp_A; /* Storage for the sub-matrix of A used during the current stage*/
	int stage, bcast_root, n_bar;  /* n/sqrt(p)   */
	int source, dest;
	MPI_Status status;
	n_bar = n/grid->q;
	Set_to_zero(local_C);
...
{	int mat_row, mat_col, grid_row, grid_col, source, coords[2];
	float *temp;
	MPI_Status status;
	if (grid->my_rank == 0)
	{	temp = (float*) malloc(Order(local_A)*sizeof(float));
