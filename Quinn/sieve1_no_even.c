...
{	int count, n, p, proc0_size, prime, size, first, global_count, high_value, i, my_rank, index, low_value;
	...
	low_value = (long long int)my_rank*(n-1)/p;			//( floor(in/p)
	high_value = (long long int)(my_rank+1)*(n-1)/p -1;
	...
	prime = 3;				// common to all as a starting point. No need of bcats() here
// debug this
	do
