...
void read_checkerboard_matrix ( dtype **subs, dtype *storage, MPI_Datatype dtype, int m, int n, MPI_Comm grid_comm)
...
{	dtype **a;       /* First factor, a matrix */
	...
	MPI_Comm col_comm;
	dtype *tmp;
	int i, j, id, m, n, nprime, p;        /* Number of processes */
	...
	dtype *btrans;
	int send_els, recv_els, src, dest, is_square;
 	MPI_Status status;
	MPI_Init (&argc, &argv);
	...
	/*	if (grid_coords[1] == 0)
		{	recv_cnt = (int *) malloc (grid_size[0] * sizeof(int));
			...
			MPI_Gatherv (b, BLOCK_SIZE(grid_coords[0],grid_size[0],n), mpitype, tmp, recv_cnt, recv_disp, mpitype, 0, col_comm);
		}
	...
	/*	if (grid_coords[0] == 0)
		{	if (grid_size[1] > 1)
			{	send_cnt = (int *) malloc (grid_size[1] * sizeof(int));
				...
				MPI_Scatterv (tmp, send_cnt, send_disp, mpitype, btrans, recv_els, mpitype, 0, row_comm);
			}
			else
			{	for (i = 0; i < n; i++)
					btrans[i] = tmp[i];
	...
	MPI_Barrier(MPI_COMM_WORLD);
	seconds += MPI_Wtime();
	MPI_Reduce (&seconds, &max_seconds, 1, mpitype, MPI_MAX, 0, MPI_COMM_WORLD);
	if (!id)
